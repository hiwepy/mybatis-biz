...
public class ResultUtils {
	...
	private static final Log logger = LogFactory.getLog(ResultUtils.class);
	...
	public static SqlSession sqlSessionBatch(Class<?> clazz) {
		...
		return (sqlSession != null) ? sqlSession : MybatisContext.getSessionFactory(clazz).openSession(ExecutorType.BATCH, false);
	}
	...
	private static SqlSession getSqlSession(Class<?> clazz, boolean isBatch) {
		...
		try {
			SqlSessionFactory sqlSessionFactory = MybatisContext.getSessionFactory(clazz);
			Configuration configuration = sqlSessionFactory.getConfiguration();
			MybatisContext definition = MybatisContext.getBindContext(configuration);
			session = isBatch ? definition.getSqlsessionBatch() : definition.getSqlSession();
		} catch (Exception e) {
			// ignored
		}
		return session;
	...
	public static SqlSession sqlSession(Class<?> clazz, boolean autoCommit) {
		...
		return (sqlSession != null) ? sqlSession : MybatisContext.getSessionFactory(clazz).openSession(autoCommit);
	}
	 ...
	 * @return TableInfo
	 */
	public static TableMetadata table(Class<?> clazz) {
		TableMetadata tableInfo = TableMetadataUtils.getTableMetadata(clazz);
		if (null == tableInfo) {
			throw new MybatisException("Error: Cannot execute table Method, ClassGenricType not found .");
		}
		return tableInfo;
	}
