...
public class MybatisXMLConfigBuilder extends BaseBuilder {
	...
	protected void settingsElement(Properties props) throws Exception {
	...
	protected void typeHandlerElement(XNode parent) throws Exception {
		if (parent != null) {
			for (XNode child : parent.getChildren()) {
				if ("package".equals(child.getName())) {
	...
	protected void mapperElement(XNode parent) throws Exception {
		/**
		 * TODO 定义集合 用来分类放置mybatis的Mapper与XML 按顺序依次遍历
         */
		if (parent != null) {
			
			/***
			 * TODO 第一步：查找mybatis配置文件填充至Set集合
			 */
			...
			for (XNode child : parent.getChildren()) {
				if ("package".equals(child.getName())) {
					...
					/**
					 * TODO 此处使用具体逻辑扫描接口类型；
					 * @see org.apache.ibatis.binding.MapperRegistry.addMappers(String, Class<?>)
					 */
					...
					if (resource != null && url == null && mapperClass == null) {
						...
						//TODO 仅先存储
						mapperClasses.add(mapperInterface);
					} else {
			...
			/***
			 * TODO 第二步：依次遍历 resource >> url >> mapper
			 */
			for (String resource : resources) {
                ErrorContext.instance().resource(resource);
                InputStream inputStream = Resources.getResourceAsStream(resource);
                //TODO 使用自定义的XMLMapperBuilder
