...
import org.apache.ibatis.jdbc.enums.SqlMethod;
 ...
 * @Date 2016-09-09
 */
public class AnnotationMapperSQLInjector implements MapperSQLInjector {
	
	private static final Log logger = LogFactory.getLog(AnnotationMapperSQLInjector.class);
	...
	public void inspectInject(MapperBuilderAssistant builderAssistant, Class<?> mapperClass) {
		...
		Set<String> mapperRegistryCache = MybatisContext.getMapperRegistryCache(builderAssistant.getConfiguration());
	...
	public void inject(MapperBuilderAssistant builderAssistant, Class<?> mapperClass) {
		...
		MybatisContext globalCache = MybatisContext.getBindContext(configuration);
		/*
		 * 驼峰设置 PLUS 配置 > 原始配置
		 */
		...
		if (modelClass != null) {
			 ...
			 * 表信息不为空的情况
			 */
			TableMetadata table = TableMetadataUtils.initTableMetadata(builderAssistant, modelClass);
			if (null != table) {
	 ...
	 * @param table
	 */
	protected void injectSql(MapperBuilderAssistant builderAssistant, Class<?> mapperClass, Class<?> modelClass, TableMetadata table) {
		/**
	...
	public void inject(Configuration configuration, MapperBuilderAssistant builderAssistant, Class<?> mapperClass,
			Class<?> modelClass, TableMetadata table) {
		// to do nothing
	}
	 ...
	 * @param table
	 */
	protected void injectInsertOneSql(Class<?> mapperClass, Class<?> modelClass, TableMetadata table) {
		/*
		...
		if (StringUtils.isNotEmpty(table.getKeyProperty())) {
			if (table.getPkStrategy() == PKStrategy.AUTO) {
				/* 自增主键 */
		...
		for (ColumnMetadata fieldInfo : fieldList) {
	 ...
	 * @param table
	 */
	protected void injectDeleteSql(Class<?> mapperClass, Class<?> modelClass, TableMetadata table) {
	 ...
	 * @param table
	 */
	protected void injectDeleteByMapSql(Class<?> mapperClass, TableMetadata table) {
	 ...
	 * @param table
	 */
	protected void injectDeleteByIdSql(boolean batch, Class<?> mapperClass, Class<?> modelClass, TableMetadata table) {
	 ...
	 * @param table
	 */
	protected void injectUpdateByIdSql(Class<?> mapperClass, Class<?> modelClass, TableMetadata table) {
	 ...
	 * @param table
	 */
	protected void injectUpdateSql(Class<?> mapperClass, Class<?> modelClass, TableMetadata table) {
	 ...
	 * @param table
	 */
	protected void injectSelectByIdSql(boolean batch, Class<?> mapperClass, Class<?> modelClass, TableMetadata table) {
	 ...
	 * @param table
	 */
	protected void injectSelectByMapSql(Class<?> mapperClass, Class<?> modelClass, TableMetadata table) {
	 ...
	 * @param table
	 */
	protected void injectSelectOneSql(Class<?> mapperClass, Class<?> modelClass, TableMetadata table) {
	 ...
	 * @param table
	 */
	protected void injectSelectListSql(SqlMethod sqlMethod, Class<?> mapperClass, Class<?> modelClass, TableMetadata table) {
	 ...
	 * @param table
	 */
	protected void injectSelectMapsSql(SqlMethod sqlMethod, Class<?> mapperClass, Class<?> modelClass, TableMetadata table) {
		String sql = String.format(sqlMethod.getSql(), sqlSelectColumns(table, true), table.getTableName(),
				sqlWhereEntityWrapper(table));
		SqlSource sqlSource = languageDriver.createSqlSource(configuration, sql, modelClass);
	 ...
	 * @param table
	 */
	protected void injectSelectObjsSql(SqlMethod sqlMethod, Class<?> mapperClass, Class<?> modelClass, TableMetadata table) {
		String sql = String.format(sqlMethod.getSql(), sqlSelectColumns(table, true), table.getTableName(),
				sqlWhereEntityWrapper(table));
		SqlSource sqlSource = languageDriver.createSqlSource(configuration, sql, modelClass);
	 ...
	 * @param table
	 */
	protected void injectSelectCountSql(Class<?> mapperClass, Class<?> modelClass, TableMetadata table) {
	 ...
	 * @return String
	 */
	protected String sqlWhereEntityWrapper(TableMetadata table) {
	 ...
	 * @return
	 */
	protected String sqlSet(TableMetadata table, String prefix) {
	...
	protected String sqlWordConvert(String convertStr) {
		MybatisContext globalConfig = MybatisContext.getBindContext(configuration);
	 ...
	 * @return
	 */
	protected String sqlSelectColumns(TableMetadata table, boolean entityWrapper) {
		...
		if (null != table.getResultMap()) {
			...
			if (_size >= 1) {
				// 字段处理
				int i = 0;
				Iterator<ColumnMetadata> iterator = fieldList.iterator();
				while (iterator.hasNext()) {
					ColumnMetadata fieldInfo = iterator.next();
					// 匹配转换内容
	 ...
	 * @return
	 */
	protected String sqlWhere(TableMetadata table, boolean space) {
		StringBuilder where = new StringBuilder();
		if (space) {
			where.append("\n<if test=\"ew!=null\">");
		}
	...
	protected String sqlWhereByMap() {
		...
		where.append("\n<if test=\"cm[k] != null\">");
		String quote = MybatisContext.getIdentifierQuote(configuration);
	 ...
	 * @return
	 */
	protected String convertIfTag(boolean ignored, ColumnMetadata fieldInfo, String prefix, boolean colse) {
		/* 忽略策略 */
		ColumnStrategy fieldStrategy = fieldInfo.getStrategy();
		if (fieldStrategy == ColumnStrategy.IGNORED) {
			if (ignored) {
				return "";
			}
			// 查询策略，使用全局策略
			fieldStrategy = MybatisContext.getBindContext(configuration).getColumnStrategy();
		}
		...
		// 验证逻辑
		if (fieldStrategy == ColumnStrategy.NOT_EMPTY) {
	...
	protected String convertIfTagIgnored(ColumnMetadata fieldInfo, boolean colse) {
		return convertIfTag(true, fieldInfo, null, colse);
	}

	protected String convertIfTag(ColumnMetadata fieldInfo, String prefix, boolean colse) {
		return convertIfTag(false, fieldInfo, prefix, colse);
	}

	protected String convertIfTag(ColumnMetadata fieldInfo, boolean colse) {
	...
	private void initSelect() {
		if (hasMappedStatement(MybatisSqlRunner.SELECT)) {
			logger.warn("MappedStatement 'SqlRunner.Select' Aalready Exists");
			return;
		}
		SqlSource sqlSource = languageDriver.createSqlSource(configuration, MybatisSqlRunner.SQLScript, Map.class);
		createSelectMappedStatement(MybatisSqlRunner.SELECT, sqlSource, Map.class);
	}
	...
	private void initCount() {
		if (hasMappedStatement(MybatisSqlRunner.COUNT)) {
			logger.warn("MappedStatement 'SqlRunner.Count' Aalready Exists");
			return;
		}
		SqlSource sqlSource = languageDriver.createSqlSource(configuration, MybatisSqlRunner.SQLScript, Map.class);
		createSelectMappedStatement(MybatisSqlRunner.COUNT, sqlSource, Integer.class);
	}
	...
	private void initInsert() {
		if (hasMappedStatement(MybatisSqlRunner.INSERT)) {
			logger.warn("MappedStatement 'SqlRunner.Insert' Aalready Exists");
			return;
		}
		SqlSource sqlSource = languageDriver.createSqlSource(configuration, MybatisSqlRunner.SQLScript, Map.class);
		createUpdateMappedStatement(MybatisSqlRunner.INSERT, sqlSource, SqlCommandType.INSERT);
	}
	...
	private void initUpdate() {
		if (hasMappedStatement(MybatisSqlRunner.UPDATE)) {
			logger.warn("MappedStatement 'SqlRunner.Update' Aalready Exists");
			return;
		}
		SqlSource sqlSource = languageDriver.createSqlSource(configuration, MybatisSqlRunner.SQLScript, Map.class);
		createUpdateMappedStatement(MybatisSqlRunner.UPDATE, sqlSource, SqlCommandType.UPDATE);
	}
	...
	private void initDelete() {
		if (hasMappedStatement(MybatisSqlRunner.DELETE)) {
			logger.warn("MappedStatement 'SqlRunner.Delete' Aalready Exists");
			return;
		}
		SqlSource sqlSource = languageDriver.createSqlSource(configuration, MybatisSqlRunner.SQLScript, Map.class);
		createUpdateMappedStatement(MybatisSqlRunner.DELETE, sqlSource, SqlCommandType.DELETE);
	}
