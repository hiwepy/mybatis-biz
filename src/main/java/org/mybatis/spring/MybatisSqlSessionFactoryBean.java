...
import org.apache.ibatis.builder.xml.MybatisXMLMapperRefresher;
...
import org.mybatis.spring.utils.PackageScanUtils;
...
public class MybatisSqlSessionFactoryBean implements FactoryBean<SqlSessionFactory>, InitializingBean, ApplicationListener<ApplicationEvent> {
	...
	protected String environment = SqlSessionFactoryBean.class.getSimpleName();
	...
	protected MybatisContext context = MybatisContext.defaults();

	// TODO 注入Mybatis全局引用对象
	public void setMybatisContext(MybatisContext context) {
	...
	protected SqlSessionFactory buildSqlSessionFactory() throws IOException {
		...
		if (hasLength(this.typeAliasesPackage)) {
			...
			if (typeAliasesPackage.contains("*")) {
				typeAliasPackageArray = PackageScanUtils.scanTypeAliasesPackage(typeAliasesPackage);
			} else {
			...
			for (String packageToScan : typeAliasPackageArray) {
				...
				try {
					...
					LOG.error("Scanned package: '" + packageToScan + "' for aliases", e);  
		            throw new NestedIOException("Scanned package: '" + packageToScan + "' for aliases", e);  
				} finally {  
		...
		if (xmlConfigBuilder != null) {
			try {
				...
				LOG.error("Failed to parse mapping resource: '" + this.configLocation + "'", ex);  
		...
		// 设置元数据相关
		context.setMetaData(dataSource);
		...
		// TODO 绑定全局参数属性
		context.bindContext(sqlSessionFactory);
	    ...
	    new MybatisXMLMapperRefresher(this.mapperLocations, configuration ).run(); 
